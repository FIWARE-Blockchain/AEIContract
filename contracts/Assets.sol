...
contract Assets is ERC721Full, Ownable{
    ...
    function isValidAsset(bytes32 uuid, bytes32[] memory _proof, bytes32 _leaf) public view returns (bool) {
        return MerkleProof.verify(_proof, stringToBytes32(payloadHash[uuid]._hash), _leaf);
    }
    ...
    //  */
    function isValidAssetEthMessage(bytes32 uuid, bytes32 _messageHash, bytes memory _signature) public view returns (bool) {
    ...
    function removeRelation(bytes32 uuid, uint index) public {
        if(payloadHash[uuid].owner != msg.sender) {
            revert("you are not the owner of the asset");
        }
    ...
    function getMetadatas(bytes32 uuid) public view returns (string[] memory) {
        string[] memory metadataHash = new string[](payloadHash[uuid].meta.length);
        
        for(uint i=0; i<payloadHash[uuid].meta.length; i++) {
    ...
    function removeMetadata(bytes32 uuid, uint index) public {
        if(payloadHash[uuid].owner != msg.sender) {
            revert("you are not the owner of the asset");
        }
    ...
    function isValidMetadata(bytes32 uuid, uint index, bytes32[] memory _proof, bytes32 _leaf) public view returns (bool) {
        return MerkleProof.verify(_proof, stringToBytes32(payloadHash[uuid].meta[index]._metainfo), _leaf);
    }
